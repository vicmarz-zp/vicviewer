...
std::wstring AntiAbuse::getWaitMessage() {
    ...
    // Si es usuario frecuente (más de 50 sesiones), mensaje diferente
    if (cachedData_.sessionCount >= 50) {
        ss << L"GRACIAS POR USAR VICVIEWER\n\n";
        ss << L"Debido al uso frecuente de este servicio,\n";
        ss << L"debe esperar un poco más entre sesiones.\n\n";
    } else {
        ss << L"SESIÓN DE CORTESÍA FINALIZADA\n\n";
    }
    ...
    ss << L"ADQUIERE UNA SUSCRIPCIÓN PARA\n";
