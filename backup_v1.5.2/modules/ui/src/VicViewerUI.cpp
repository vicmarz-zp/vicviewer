...
void connectViewer(MainWindowState* state) {
    ...
    std::string codeUtf8 = wideToUtf8(code);
    vic::logging::global().log(vic::logging::Logger::Level::Info, "[UI] connectViewer: resolviendo codigo " + codeUtf8);
    
    auto info = state->matchmaker->resolveCode(codeUtf8);
    if (!info) {
    ...
    vic::logging::global().log(vic::logging::Logger::Level::Info, "[UI] connectViewer: codigo resuelto OK, configurando frame callback");
    ...
    vic::logging::global().log(vic::logging::Logger::Level::Info, "[UI] connectViewer: Timer de timeout iniciado (15s)");
    
    vic::logging::global().log(vic::logging::Logger::Level::Info, "[UI] connectViewer: llamando viewerSession->connect()");
    bool connectResult = state->viewerSession->connect(codeUtf8);
    vic::logging::global().log(vic::logging::Logger::Level::Info, 
    ...
    state->viewerSession->enableAutoReconnect(codeUtf8);
...
void createControls(MainWindowState* state, HWND parent) {
    ...
    state->hostStatus = CreateWindowW(L"STATIC", L"",
        ...
        MARGIN, contentY + 140, contentWidth, 20,
        parent, nullptr, GetModuleHandle(nullptr), nullptr);
    ...
    state->hostMetricsLabel = CreateWindowW(L"STATIC", L"",
        WS_CHILD | WS_VISIBLE | SS_CENTER,
        MARGIN, contentY + 165, contentWidth, 20,
        parent, nullptr, GetModuleHandle(nullptr), nullptr);
    ...
    state->bannerStatic = CreateWindowW(L"STATIC", L"",
        ...
        MARGIN, contentY + 195, BANNER_WIDTH, BANNER_HEIGHT,
        parent, nullptr, GetModuleHandle(nullptr), nullptr);
    ...
    SendMessage(state->viewerCodeEdit, EM_SETCUEBANNER, TRUE, (LPARAM)L"Codigo");
    ...
    // Checkbox para reconectar al reiniciar
    ...
    // Checkbox para no generar codigo al inicio
...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (PtInRect(&state->tabHostRect, pt) && state->currentTab != TabMode::Host) {
            ...
            // Cargar configuración guardada al entrar a la pestaña Servicio
            loadServiceConfig(state);
            // Deshabilitar checkbox "no auto code" si no es suscriptor pagado
            if (state->serviceNoAutoCode && state->isFreeMode) {
                EnableWindow(state->serviceNoAutoCode, FALSE);
